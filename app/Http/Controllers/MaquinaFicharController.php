<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use App\Models\Dispositivo;
use App\Models\MaquinaFichar;
use App\Exports\MaquinasFicharExport;
use Illuminate\Support\Facades\Request as RequestFacade;
use Inertia\Inertia;
use Maatwebsite\Excel\Facades\Excel;

class MaquinaFicharController extends Controller
{
    
    public function store(){

        $data = RequestFacade::validate([

            'nombre_dispositivo' => ['required','unique:dispositivos'],
            'numero_serie' => ['nullable'],
            'conexion' => ['required'],
            'zona_trabajo' => ['required'],
            'tipo_dispositivo' => ['required'],
            'ip_principal' => ['required'],
            'observaciones' => ['nullable']
        ],
        
        [
            'nombre_dispositivo.required' => 'El nombre de dispositivo es obligatorio',
            'tipo_dispositivo.required' => 'El tipo de dispositivo es obligatorio',
            'conexion.required' => 'La conexión es obligatoria',
            'ip_principal.required' => 'La IP Principal es obligatoria',
            'zona_trabajo.required' => 'La zona de trabajo es obligatoria'

        ]);

        $dispositivo = Dispositivo::create([

            'nombre_dispositivo' => $data['nombre_dispositivo'], 
            'numero_serie' => $data['numero_serie'],
            'tipo_dispositivo' => $data['tipo_dispositivo'],
            'conexion' => $data['conexion'],
            'zona_trabajo' => $data['zona_trabajo']

        ]);

        $maquinafichar = MaquinaFichar::create([

            'nombre_dispositivo' => $data['nombre_dispositivo'],
            'id_dispositivo' => $dispositivo->id,
            'ip_principal' => $data['ip_principal'],
            'observaciones' => $data['observaciones']

        ]);

        return redirect('/dispositivos')->with('mensaje','Máquina de fichar añadida correctamente');

    }

    public function edit($id){

        return Inertia::render('Dispositivos/MaquinaFichar/Edit',[

            'dispositivo' => Dispositivo::find($id),
            'maquinafichar' => MaquinaFichar::where('id_dispositivo',$id)->first()

        ]);

    }

    public function update(Request $request,$id){

        $request->validate([

            'nombre_dispositivo' => ['required'],
            'numero_serie' => ['nullable'],
            'tipo_dispositivo' => ['required'],
            'conexion' => ['required'],
            'zona_trabajo' => ['required'],
            'ip_principal' => ['required'],
            'observaciones' => ['nullable']
        ]);


        $dispositivo = Dispositivo::where('id',$id);

        $dispositivo->update([

            'nombre_dispositivo' => $request->nombre_dispositivo,
            'numero_serie' => $request->numero_serie,
            'tipo_dispositivo' => $request->tipo_dispositivo,
            'conexion' => $request->conexion,
            'zona_trabajo' => $request->zona_trabajo

        ]);

        $maquinafichar = MaquinaFichar::where('id_dispositivo',$id);

        $maquinafichar->update([

            'nombre_dispositivo' => $request->nombre_dispositivo,
            'ip_principal' => $request->ip_principal,
            'observaciones' => $request->observaciones,

        ]);

        return redirect('/dispositivos')->with('mensaje','Máquina de fichar actualizada correctamente');
    }


    public function delete($id){

        Dispositivo::find($id)->delete();

        return redirect('/dispositivos')->with('mensaje', 'Máquina de fichar eliminada correctamente');

    }

    public function show($id) {

        return Inertia::render('Dispositivos/MaquinaFichar/Show',[

            'dispositivo' => Dispositivo::find($id),
            'maquinafichar' => MaquinaFichar::where('id_dispositivo',$id)->first(),
        ]);
    }

    public function exportar()
    {
        return Excel::download(new MaquinasFicharExport,'maquinas-fichar.xlsx');

    }


}
